...
A *block* in Ruby is simply a sequence of code enclosed by `{` and `}` (or alternatively by `do` and `end`). You can optionally supply a block with local variables by enclosing them with `|` and `|` at the start.
...
Blocks are NOT objects, but merely syntactical constructions in Ruby. You cannot assign them to variables, or store them in arrays. Pretty much all you can do with them is pass them to methods. Certain methods in Ruby expect and require a block to be passed to them.

Examples of passing blocks to methods:
